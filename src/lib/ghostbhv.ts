...
export class GhostBehavior implements Behavior<AbstractMesh> {
    ...
    get targetMesh(): Nullable<AbstractMesh> { return this.targetMesh; }
    set targetMesh(target: Nullable<AbstractMesh>) {
        this._targetMesh = target;
        if (this.attachedNode) this.reset();
        if (this._targetMesh) this.#setupObservers(); else this.#clearObservers();
    }
    ...
    reset() {
        ...
        this._attachedNode.setEnabled(this._targetMesh !== null);
        if (this._targetMesh) {
